...
class HtmlInput
  ...
  # def as_date
  #   @opts[:type]         = 'date'
  #   @opts[:style]        = 'width: 160px; height: 40px;'
  #   @opts[:value]        = @opts[:value].short rescue @opts[:value]
  #   @opts[:autocomplete] = :off

  #   if @opts[:placeholder] == 'today' && !@opts[:value]
  #     @opts[:type]    = 'text'
  #     @opts[:onclick] = "this.type='date'"
  #   end

  #   ret = @opts.tag(:input)

  #   # bypass date controll
  #   return ret

  #   if @opts[:value].present? && !@opts.delete(:no_ago)
  #     ret += '<span class="date-ago"> &bull; %s</span>' % Time.ago(Time.parse(@opts[:value]))
  #   end

  #   fmt = Lux.current.var.date_format || 'YYYY-MM-DD'

  #   ret + %[<script>new Pikaday({ field: document.getElementById('#{@opts [:id]}'), format: "#{fmt}" }); </script>]
  # end
