module LuxDeploy
  module Release
    ...
    def ensure_layout(ctx)
      cmd = "if [ -d #{LuxDeploy.sh(ctx.path)} ]; then test -w #{LuxDeploy.sh(ctx.path)}; else sudo mkdir -p #{LuxDeploy.sh(ctx.path)}/releases #{LuxDeploy.sh(ctx.path)}/shared && sudo chown -R #{LuxDeploy.sh(ctx.remote_user)}:#{LuxDeploy.sh(ctx.remote_user)} #{LuxDeploy.sh(ctx.path)}; fi && mkdir -p #{LuxDeploy.sh(ctx.path)}/releases #{LuxDeploy.sh(ctx.path)}/shared/log #{LuxDeploy.sh(ctx.path)}/shared/tmp"
      ctx.ssh.ssh!(cmd, category: :preflight, summary: 'cannot ensure release layout')
    end

    def create(ctx)
      ctx.release = LuxDeploy.now_ts
      ctx.ssh.ssh!("mkdir -p #{release_path(ctx)}", category: :source, summary: 'cannot create release dir')
      ctx.release
    end
    ...
    def sync_source(ctx)
      if ctx.config[:branch]
        ...
        ctx.ssh.ssh!(cmd, category: :source, summary: 'git clone failed')
      else
    ...
    def symlink_shared(ctx)
      ...
      ctx.ssh.ssh!(cmd, category: :source, summary: 'cannot link shared release files')
    end

    def bundle_install(ctx)
      cmd = "cd #{release_path(ctx)} && #{LuxDeploy.sh(ctx.bundle)} install --deployment --without development test"
      ctx.ssh.ssh!(cmd, category: :source, summary: 'bundle install failed')
    end

    def migrate(ctx)
      cmd = "cd #{release_path(ctx)} && #{LuxDeploy.sh(ctx.bundle)} exec lux db:am"
      ctx.ssh.ssh!(cmd, category: :db, summary: 'database migration failed')
    end
    ...
    def swap_current(ctx)
      ...
      ctx.ssh.ssh!(cmd, category: :systemd, summary: 'atomic current swap failed')
    ...
    def prune(ctx)
      ...
      ctx.ssh.ssh!(cmd, category: :source, summary: 'release prune failed')
    end

    def rollback(ctx)
      releases_cmd = "cd #{LuxDeploy.sh(ctx.path)}/releases && ls -1dt */ 2>/dev/null | sed -e 's#/##' | sed -n '2p'"
      result = ctx.ssh.ssh(releases_cmd)
      ...
      ctx.ssh.ssh!(cmd, category: :systemd, summary: 'rollback current swap failed')
