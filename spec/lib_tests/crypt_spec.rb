...
describe Lux::Utils::Crypt do
  describe '.encrypt / .decrypt' do
    it 'encrypts and decrypts a simple string' do
      ...
      expect(Lux::Utils::Crypt.decrypt(encrypted)).to eq('abc')
    end

    it 'encrypts and decrypts complex data' do
      data = { 'users' => [1, 2, 3], 'name' => 'test' }
      encrypted = Lux::Utils::Crypt.encrypt(data)
      expect(Lux::Utils::Crypt.decrypt(encrypted)).to eq(data)
    end

    it 'encrypts with password and requires it for decryption' do
      encrypted = Lux::Utils::Crypt.encrypt('abc', password: 'foo')
      expect { Lux::Utils::Crypt.decrypt(encrypted) }.to raise_error(StandardError)
      expect(Lux::Utils::Crypt.decrypt(encrypted, password: 'foo')).to eq('abc')
    end
    ...
    it 'encrypts with TTL and rejects expired tokens' do
      ...
      expect(Lux::Utils::Crypt.decrypt(future_str)).to eq('foo')
      expect { Lux::Utils::Crypt.decrypt(expired_str) }.to raise_error(StandardError)
    end

    it 'returns nil for expired tokens with unsafe option' do
      expired = Lux::Utils::Crypt.encrypt('foo', ttl: -10)
      expect(Lux::Utils::Crypt.decrypt(expired, unsafe: true)).to be_nil
    end
  end
  ...
  describe '.sha1' do
    it 'returns a consistent hex digest' do
      ...
      expect(Lux::Utils::Crypt.sha1('test')).to eq(result)
    end

    it 'returns different digests for different inputs' do
      expect(Lux::Utils::Crypt.sha1('a')).not_to eq(Lux::Utils::Crypt.sha1('b'))
    end
  end
  ...
  describe '.md5' do
    it 'returns a consistent hex digest' do
      ...
      expect(Lux::Utils::Crypt.md5('test')).to eq(result)
    end

    it 'returns different digests for different inputs' do
      expect(Lux::Utils::Crypt.md5('a')).not_to eq(Lux::Utils::Crypt.md5('b'))
    end
  end
  ...
  describe '.random' do
    ...
    it 'defaults to 32 characters' do
      expect(Lux::Utils::Crypt.random.length).to eq(32)
    end
  end
  ...
  describe '.short_encrypt / .short_decrypt' do
    it 'encrypts and decrypts data with short tokens' do
      ...
      expect(encrypted).to be_a(String)
      expect(Lux::Utils::Crypt.short_decrypt(encrypted)).to eq('secret_data')
    end

    it 'rejects expired short tokens' do
      encrypted = Lux::Utils::Crypt.short_encrypt('secret_data', -10)
      expect { Lux::Utils::Crypt.short_decrypt(encrypted) }.to raise_error(ArgumentError, /expired/)
    end

    it 'rejects tampered tokens' do
      encrypted = Lux::Utils::Crypt.short_encrypt('secret_data')
      tampered = 'XXXXXXXX' + encrypted[8..]
      expect { Lux::Utils::Crypt.short_decrypt(tampered) }.to raise_error(ArgumentError, /Invalid check/)
    end
  end
  ...
  describe '.simple_encode / .simple_decode' do
    it 'encodes and decodes strings with ROT13+base64' do
      ...
      expect(Lux::Utils::Crypt.simple_decode(encoded)).to eq(original)
    end
  end
end