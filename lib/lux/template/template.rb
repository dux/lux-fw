module Lux
  class Template
    class << self
      ...
      def find_layout root, layout_template
        ...
        if path
          ...
          Lux.error 404, Lux.mode.debug?('404 Not Found') { %[Layout path for #{layout_template} not found. Looked in #{base1} & #{base2}] }
        end
      end
    end
    ...
    def compile_template template
      ...
      unless @template
        Lux.error 404, Lux.mode.debug?('404 Not Found') { %[Lux::Template "#{template}.{erb,haml}" not found] }
      end
