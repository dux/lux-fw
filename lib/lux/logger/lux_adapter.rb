...
module Lux
  ...
  def default_logger
    @default_logger ||= begin
      ...
      # Default uses Lux.debug? so the logger works before Lux.boot! has populated config.
      level = Lux.config.key?(:log_level) ? Lux.config[:log_level] : (Lux.debug? ? :info : :error)
