...
Commands:
  ...
  lux sysd uninstall         Stop and remove all services for this app
...
Required .env variables:
  ...
  PORT_RANGE=3000-3003       Port range creates separate service for each port

Generated files:
  #{SYSD_DIR}/lux-web-{app}.service       Systemd web service (single port)
  #{SYSD_DIR}/lux-web-{app}-{port}.service  Systemd web service (per port, if PORT_RANGE)
  ...
  def sysd_generate
    ...
    port_str = ENV['PORT_RANGE'] || ENV.fetch('PORT', '3000')
  ...
  def generate_web_service(app_name, domain, port_str, ports)
    ...
    ports.each do |port|
      suffix = ports.size > 1 ? "-#{port}" : ""
      file = "#{SYSD_DIR}/lux-web-#{app_name}#{suffix}.service"
      service_name = "lux-web-#{app_name}#{suffix}"

      content = <<~SERVICE
# Lux Web Service - Generated by: lux sysd generate
# App: #{app_name} | Domain: #{domain} | Port: #{port}
#
# Install:
#   sudo ln -sf #{pwd}/config/sysd/#{service_name}.service /etc/systemd/system/
#   sudo systemctl daemon-reload
#   sudo systemctl enable #{service_name}
#   sudo systemctl start #{service_name}

[Unit]
Description=Lux Web #{app_name} (#{domain}:#{port})
...
ExecStart=#{bundle_path} exec lux server -e production
Restart=always
RestartSec=5
...
WantedBy=multi-user.target
      ...
      count += write_config(file, content)
    end
...
server {
  ...
  class ServiceTUI
    ...
    def available_services
      ...
      lux_services.each do |name|
        next if seen.include?(name)
        seen << name
        status = get_status(name)
        display = "#{name} [#{status}]"
        @service_map[display] = { service: name, config: { actions: %w[status start stop restart log] } }
        services << display
      end
