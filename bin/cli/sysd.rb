...
Generated files:
  ...
  def sysd_generate
    ...
    generated = 0
    generated += generate_web_service(app_name, domain, port)
    generated += generate_job_service(app_name)
    generated += generate_caddy(app_name, domain, port)
    generated += generate_nginx(app_name, domain, port)
  ...
  def generate_web_service(app_name, domain, port)
    user = ENV['USER']
    pwd = Dir.pwd
    bundle_path = `which bundle`.strip
    service_name = "lux-web-#{app_name}"
    file = "#{SYSD_DIR}/#{service_name}.service"
...
WantedBy=multi-user.target
  ...
  def generate_caddy(app_name, domain, port)
...
#{domain}, *.#{domain} {
  ...
  def generate_nginx(app_name, domain, port)
